...
uint16_t  BLETransceiver::m_conn_handle                          = BLE_CONN_HANDLE_INVALID;
...
void BLETransceiver::advertise(void)
{
  uint32_t             err_code;
...
bool BLETransceiver::sendData(uint8_t* arg_au8_dataToSend, uint8_t arg_u8_dataLength)
{
    uint32_t err_code;

    err_code = ble_nus_send_string(&_m_nus, arg_au8_dataToSend, arg_u8_dataLength);
...
void BLETransceiver::onBleEvt(ble_evt_t * p_ble_evt)
{
    ...
    LOG_INFO_LN("BLE event received : %d", p_ble_evt->header.evt_id);
