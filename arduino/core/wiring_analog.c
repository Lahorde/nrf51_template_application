...
void turn_On_PPI_to_GPIO_for_PWM(uint32_t ulPin, uint32_t gpiote_channel, NRF_TIMER_Type* Timer, uint32_t CC_channel)
{
	...
	if ((chan_0 != 255) && (chan_1 != 255))
	{
		...
		ppi_channel_assign(chan_0, &((*Timer).EVENTS_COMPARE[CC_channel]), &NRF_GPIOTE->TASKS_OUT[gpiote_channel]);
		...
		ppi_channel_assign(chan_1, &((*Timer).EVENTS_COMPARE[3]), &NRF_GPIOTE->TASKS_OUT[gpiote_channel]);
		// Save PPI channel number
...
void turn_Off_GPIOTE_PPI_from_GPIO(uint32_t ulPin)
{
	...
	{
		ppi_channel_unassign(PPI_Channels_Occupied[ulPin][0]);
		PPI_Channels_Occupied[ulPin][0] = 255;
	}

	if (PPI_Channels_Occupied[ulPin][1] < 255)
	{
		ppi_channel_unassign(PPI_Channels_Occupied[ulPin][1]);
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if (!GZLL_used && Timer2_Compare_Unit_Occupied_by_Pin[0] == 255)
				{
